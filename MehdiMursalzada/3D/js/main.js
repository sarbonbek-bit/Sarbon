...
function drawTarget(x, y, z, size = 60) {
    ...
    targets.push(t);
    targetsData.push({ x, y, z, size, alive: true });
}
...
function update() {
    ...
    if (lock) {
        ...
        pawn.rx = Math.max(-57, Math.min(57, pawn.rx));
    }
    ...
    let cosX = Math.cos(myBulletsData[i].rx * DEG);
    let sinX = Math.sin(myBulletsData[i].rx * DEG);
    let cosY = Math.cos(myBulletsData[i].ry * DEG);
    let sinY = Math.sin(myBulletsData[i].ry * DEG);

    myBulletsData[i].x += speed * sinY * cosX;
    myBulletsData[i].z += -speed * cosY * cosX;
    myBulletsData[i].y += speed * sinX;

    myBullets[i].style.transform =
        `translate3d(${600 + myBulletsData[i].x - 25}px,
...
function checkBulletHits() {
    for (let b = 0; b < myBulletsData.length; b++) {
        for (let t = 0; t < targetsData.length; t++) {

            if (!targetsData[t].alive) continue;

            let dxT = myBulletsData[b].x - targetsData[t].x;
            let dyT = myBulletsData[b].y - targetsData[t].y;
            let dzT = myBulletsData[b].z - targetsData[t].z;
