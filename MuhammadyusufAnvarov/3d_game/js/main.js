...
let myRoom = [
    ...
    [0, 100, 0, 90, 0, 0, ROOM_WIDTH, ROOM_DEPTH, "#3d3d4d", 1, "url('textures/castle_floor.jpg')", "castle-floor"],
    

    [0, -ROOM_HEIGHT + 50, 0, -90, 0, 0, ROOM_WIDTH, ROOM_DEPTH, "#1a1a25", 1, "url('textures/castle_ceiling.jpg')", "castle-ceiling"],
    

    [0, -ROOM_HEIGHT/2 + 75, -ROOM_DEPTH/2, 0, 0, 0, ROOM_WIDTH, ROOM_HEIGHT + 50, "#4a4a5a", 1, "url('textures/castle_wall.jpg')", "castle-wall"],
    

    [0, -ROOM_HEIGHT/2 + 75, ROOM_DEPTH/2, 0, 180, 0, ROOM_WIDTH, ROOM_HEIGHT + 50, "#4a4a5a", 1, "url('textures/castle_wall.jpg')", "castle-wall"],
    

    [-ROOM_WIDTH/2, -ROOM_HEIGHT/2 + 75, 0, 0, 90, 0, ROOM_DEPTH, ROOM_HEIGHT + 50, "#5a5a6a", 1, "url('textures/castle_wall.jpg')", "castle-wall"],
    

    [ROOM_WIDTH/2, -ROOM_HEIGHT/2 + 75, 0, 0, -90, 0, ROOM_DEPTH, ROOM_HEIGHT + 50, "#5a5a6a", 1, "url('textures/castle_wall.jpg')", "castle-wall"],
    

    [0, 70, 0, 90, 0, 0, 300, 300, "#2d2d3d", 1, "url('textures/castle_floor.jpg')", "castle-floor"],
];
...
function update() {
    ...
    for (let i = 0; i < myBullets.length; i++) {
        ...
        let bulletSpeed = 15;
        let dzb = -bulletSpeed * Math.cos(myBulletsData[i].ry * DEG);
        let dxb = bulletSpeed * Math.sin(myBulletsData[i].ry * DEG);

        myBulletsData[i].x += dxb;
        myBulletsData[i].z += dzb;
        

        myBullets[i].style.transform = `translate3d(${center.x + myBulletsData[i].x - 15}px, ${center.y + myBulletsData[i].y - 15}px, ${myBulletsData[i].z}px) rotateX(${myBulletsData[i].rx}deg) rotateY(${-myBulletsData[i].ry}deg)`;
    }
...
function drawMyWorld(squares, name) {
    for (let i = 0; i < squares.length; i++) {
        ...
        if (squares[i][10]) {
            ...
            mySquare1.style.backgroundSize = "256px 256px";
            mySquare1.style.backgroundRepeat = "repeat";
        } else if (squares[i][11]) {
           
            mySquare1.classList.add(squares[i][11]);
        } else {
            mySquare1.style.backgroundColor = squares[i][8];
        }
        
        mySquare1.style.transform = `translate3d(${center.x + squares[i][0] - squares[i][6] / 2}px, ${center.y + squares[i][1] - squares[i][7] / 2}px, ${squares[i][2]}px) rotateX(${squares[i][3]}deg) rotateY(${squares[i][4]}deg) rotateZ(${squares[i][5]}deg)`;
        mySquare1.style.opacity = squares[i][9];
        mySquare1.style.backfaceVisibility = "hidden";
...
function drawMyBullet(num) {
    ...
    myBullet.style.width = `30px`;
    myBullet.style.height = `30px`;
    myBullet.style.borderRadius = `50%`;
    myBullet.style.background = `radial-gradient(circle at 30% 30%, #ffdd88, #cc8800, #664400)`;
    myBullet.style.boxShadow = `0 0 15px #ffaa00, 0 0 30px #ff6600`;
    myBullet.style.transform = `translate3d(${center.x + pawn.x - 15}px, ${center.y + pawn.y - 15}px, ${pawn.z}px) rotateX(${pawn.rx}deg) rotateY(${-pawn.ry}deg)`;
